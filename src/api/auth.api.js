...
export const EMAIL_TIMEOUT = 30_000; // 30s — email sending operations (SMTP over cloud network)
...
export const forgotPasswordApi    = (data)         => api.post('/auth/forgot-password',        data, { timeout: EMAIL_TIMEOUT });
export const resetPasswordApi     = (data, token)  => api.post(`/auth/reset-password?token=${token}`, data, { timeout: AUTH_TIMEOUT });
export const sendVerificationOtpApi = ()           => api.get('/auth/send-verification-otp',       { timeout: EMAIL_TIMEOUT });
